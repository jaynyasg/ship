-- Idempotent repair for deployments that reached assistant upload code with
-- partially-applied file indexing schema.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'files_assistant_indexing_status_check'
  ) THEN
    ALTER TABLE files
