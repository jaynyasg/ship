...
> **Status:** Phase 1 (baseline) complete. Phase 2 (improvements) gated; awaiting checkpoint approval to begin.
...
**Approach:** Column projection — list-view queries return only the needed columns (id, document_type, title, properties, timestamps, ticket_number, parent_id, workspace_id) and explicitly EXCLUDE `content` (JSONB) and `yjs_state` (BYTEA). The issues route's inclusion of `d.content` is the single line that needs to be removed.
...
|---|---|---|---|
| 1. Type Safety | 399 grep violations; 93.47% type-coverage | ≥25% violation reduction | (To be measured after U11) |
| 2. Bundle Size | 2,073 KB main chunk | ≥15% total OR ≥20% initial via splitting | (To be measured after U12) |
| 3. API Response Time | documents P95 c=25 = 283 ms; issues P95 c=25 = 192 ms | ≥20% P95 on ≥2 endpoints | (To be measured after U13) |
| 4. DB Query Efficiency | weeks query has 7 correlated subqueries | ≥20% query count OR ≥50% slowest query | (To be measured after U14) |
| 5. Test Coverage | 451 unit + 73+ E2E; 6 empty/silent-pass tests | 3 new tests OR 3 flaky fixes | (To be addressed in U15) |
| 6. Runtime Error Handling | No top-level ErrorBoundary; no global error handlers | Fix 3 gaps, ≥1 data-loss scenario | (To be addressed in U16) |
| 7. Accessibility | 0 Critical, 2 Serious axe violations | +10 Lighthouse OR 0 Critical/Serious on top 3 | (To be addressed in U17) |
