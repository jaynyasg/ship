...
| 17 | **TypeScript itself is a major version behind** (5.9.3 → 6.0.3 available). | Medium | Supply chain freshness signal. Major upgrade is non-trivial. |
...
| 20 | **`api/` build script uses bash-only `cp` command.** `api/package.json` has `"build": "tsc && cp src/db/schema.sql dist/db/schema.sql && cp -r src/db/migrations dist/db/migrations"` — `cp` is a POSIX command that doesn't exist in Windows cmd.exe (where pnpm runs scripts by default). When invoked indirectly (e.g., via Playwright's `globalSetup` which calls `pnpm build:api`), the failure cascades and blocks the E2E test suite from running. | Medium | Blocks Windows developers from running `pnpm test:e2e` (which depends on `pnpm build:api`). Workarounds: (a) set `pnpm config set script-shell pwsh.exe` (uses PowerShell which has `cp` aliased to `Copy-Item`); (b) run the build steps manually: `tsc; Copy-Item src/db/schema.sql dist/db/schema.sql; Copy-Item src/db/migrations dist/db/migrations -Recurse`; (c) adopt `shx` package for cross-platform file commands in scripts (out of audit scope). Combined with findings #1 and #19, this is the third bash-only script blocker. Ship is well-tested on macOS/Linux but has zero Windows-developer support. |
